...
PyObject * MGLContext_meth_program(MGLContext * self, PyObject * args) { TRACE_VARAGS
	PyObject * shaders[5];
	PyObject * varyings;
	...
	if (!program_obj) {
		PyErr_Format(module_error, "cannot create program");
		Py_DECREF(program);
		return 0;
	}
	...
	for (int i = 0; i < 5; ++i) {
		...
		if (!PyUnicode_Check(shaders[i])) {
			PyErr_Format(PyExc_TypeError, "%s must be a str not %s", SHADER_NAME[i], shaders[i]->ob_type->tp_name);
			Py_DECREF(program);
			return 0;
		}
		...
		if (!shader_obj) {
			PyErr_Format(module_error, "cannot create shader");
			Py_DECREF(program);
			return 0;
		}
		...
		if (!compiled) {
			...
			PyObject_CallFunctionObjArgs(compiler_error, name, shaders[i], info, 0);
			Py_DECREF(name);
			Py_DECREF(info);
			Py_DECREF(program);
			free(log_text);
	...
	if (varyings_len) {
		...
		for (int i = 0; i < varyings_len; ++i) {
			...
			if (!PyUnicode_Check(varying)) {
				PyErr_Format(module_error, "expected str, got %s", varying->ob_type->tp_name);
				Py_DECREF(varyings);
				Py_DECREF(program);
				delete[] varyings_str;
				return 0;
	...
	if (!linked) {
		...
		PyObject * info = PyUnicode_FromStringAndSize(log_text, log_len);
		PyObject_CallFunctionObjArgs(compiler_error, shaders[0], shaders[1], shaders[2], shaders[3], shaders[4], info, 0);
		Py_DECREF(info);
		Py_DECREF(program);
		free(log_text);
		return 0;
...
PyTypeObject * MGLProgram_define(int version_code) {
	...
	PyType_Slot MGLProgram_slots[] = {
		{Py_tp_methods, dup(MGLProgram_methods)},
		{0},
	};

	PyType_Spec MGLProgram_spec = {"moderngl.mgl.Program", sizeof(MGLProgram), 0, Py_TPFLAGS_DEFAULT, MGLProgram_slots};
	return (PyTypeObject *)PyType_FromSpec(&MGLProgram_spec);
}
