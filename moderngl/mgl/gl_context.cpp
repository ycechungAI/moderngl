...
bool GLContext::load(bool standalone) {
	...
	if (standalone) {
		...
		if (!inst) {
			PyErr_Format(module_error, "module handle is null");
			return false;
		}
		...
		if (!registered) {
			...
			if (!RegisterClass(&wndClass)) {
				PyErr_Format(module_error, "cannot register window class");
				return false;
			}
		...
		if (!hwnd) {
			PyErr_Format(module_error, "cannot create window");
			return false;
		}
		...
		if (!hdc) {
			PyErr_Format(module_error, "cannot create device content");
			return false;
		}
		...
		if (my_CreateContextAttribs && my_ChoosePixelFormat) {
			...
			if (!my_ChoosePixelFormat(hdc, empty1, empty2, 1, &pixelformat, &num_formats)) {
				PyErr_Format(module_error, "cannot choose pixel format");
				return false;
			}

			if (!num_formats) {
				PyErr_Format(module_error, "no pixel formats available");
				return false;
			}

			if (!SetPixelFormat(hdc, pixelformat, &pfd)) {
				PyErr_Format(module_error, "cannot set pixel format");
				return false;
			}
		...
		if (!hrc) {
			PyErr_Format(module_error, "cannot create OpenGL context");
			return false;
		}
		...
		if (!make_current) {
			PyErr_Format(module_error, "cannot select OpenGL context");
			return false;
		}
	}
	...
	if (!hrc) {
		PyErr_Format(module_error, "cannot detect context");
		return false;
	}
	...
	if (!hdc) {
		PyErr_Format(module_error, "cannot detect device content");
		return false;
	}
	...
	if (!hwnd) {
		PyErr_Format(module_error, "cannot detect window");
		return false;
	}
