...
PyObject * MGLContext_meth_vertex_array(MGLContext * self, PyObject * args) { TRACE_VARAGS
	PyObject * program;
	PyObject * content;
	...
	if (!vertex_array->vertex_array_obj) {
		PyErr_Format(module_error, "cannot create vertex array");
		return 0;
	}
	...
	for (int i = 0; i < content_len; ++i) {
		...
		if (item_len < 3) {
			PyErr_Format(module_error, "err1");
			return 0;
		}
		...
		if (!PyUnicode_Check(format)) {
			PyErr_Format(module_error, "err2");
			return 0;
		}

		PyObject * format_split = PyObject_CallFunctionObjArgs(split_format, format, 0);
		if (!format_split) {
			return 0;
		}
		...
		int nodes_len = PySequence_Fast_GET_SIZE(nodes);
		int nodes_cnt = 0;
		...
		for (int j = 2; j < item_len; ++j) {
			...
			if (!PyUnicode_Check(name)) {
				PyErr_Format(module_error, "err4");
				return 0;
			}
...
PyObject * MGLVertexArray_meth_render(MGLVertexArray * self, PyObject * args) { TRACE_VARAGS
	PyObject * mode;
	int vertices;
	int first;
	int instances;
...
PyObject * MGLVertexArray_meth_transform(MGLVertexArray * self, PyObject * args) { TRACE_VARAGS
	PyObject * buffer;
	PyObject * mode;
	int vertices;
	int first;
...
int MGLVertexArray_set_ibo(MGLVertexArray * self, PyObject * value) { TRACE_SETTER
...
PyTypeObject * MGLVertexArray_define(int version_code) {
	PyMethodDef MGLVertexArray_methods[] = {
		{"render", (PyCFunction)MGLVertexArray_meth_render, METH_VARARGS, 0},
		{"transform", (PyCFunction)MGLVertexArray_meth_transform, METH_VARARGS, 0},
		{0},
	};
	...
	PyType_Slot MGLVertexArray_slots[] = {
		{Py_tp_methods, dup(MGLVertexArray_methods)},
		{Py_tp_getset, dup(MGLVertexArray_getset)},
		{0},
	};

	PyType_Spec MGLVertexArray_spec = {"moderngl.mgl.VertexArray", sizeof(MGLVertexArray), 0, Py_TPFLAGS_DEFAULT, MGLVertexArray_slots};
	return (PyTypeObject *)PyType_FromSpec(&MGLVertexArray_spec);
}
